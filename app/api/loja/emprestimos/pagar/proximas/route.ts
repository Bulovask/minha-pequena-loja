...
import { pagarPrimeirasParcelasDoEmprestimo } from "@/functions/emprestimo/pagarPrimeirasParcelasDoEmprestimo";
...
export async function POST(req: NextRequest) {
  try {
    await dbConnect();
    // Força o registro do modelo Usuario caso haja algum populate pendente na sessão
    ...
    const sucesso = pagarPrimeirasParcelasDoEmprestimo(
      sessao,
      emprestimo,
      Number(quantidade)
    );
